...
function startPersonalFolderInlineCreate(parentId=null){
  if(!personalOwnedFolderContainer)return;
  document.querySelector('[data-personal-folder-inline]')?.remove();
  const parentNode=parentId?document.querySelector(`[data-personal-folder-node="${parentId}"]`):null;
  const target=parentNode?.querySelector(':scope > .personal-folder-subtree')||personalOwnedFolderContainer;
  target.insertAdjacentHTML('beforeend','<form class="personal-folder-inline-editor" data-personal-folder-inline><span class="personal-folder-inline-icon" aria-hidden="true">□</span><span class="input-wrap input-middle"><input class="input" name="personalFolderName" maxlength="30" required placeholder="文件夹名称" aria-label="文件夹名称" autocomplete="off" /></span></form>');
  const form=target.querySelector('[data-personal-folder-inline]');
  const input=form.querySelector('[name="personalFolderName"]');
  let cancelled=false;
  const cancel=()=>{cancelled=true;form.remove()};
  form.addEventListener('submit',event=>{event.preventDefault();const name=input.value.trim();if(!name)return cancel();const parent=parentId?findPersonalFolder(personalCustomFolders,parentId):null,siblings=parent?(parent.children||(parent.children=[])):personalCustomFolders;if(siblings.some(folder=>folder.name===name)){showToast('同级文件夹名称已存在');input.focus();input.select();return}const folder={id:`personal-folder-${Date.now()}`,name,count:0,children:[]};siblings.push(folder);localStorage.setItem(personalFolderStorageKey,JSON.stringify(personalCustomFolders));renderPersonalCustomFolders();const button=document.querySelector(`[data-personal-folder="${folder.id}"]`);if(button)activatePersonalFolder(button);showToast(`已创建文件夹“${name}”`)});
