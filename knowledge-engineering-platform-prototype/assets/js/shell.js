...
const resourceChildren = [
  ['resource-overview', '资源文档概览', 'pages/resource-overview.html', 'overview'],
  ...
  ['resource-catalog-design', '资源目录管理', 'pages/resource-catalog-design.html', 'management'],
  ['metadata-standards', '元数据标准管理', 'pages/metadata-standards.html', 'management'],
...
export function renderShell() {
  ...
  if (resourcePages.has(page)) {
    ...
    const overviewNavigation = `<a class="resource-nav-item ${page === 'resource-overview' ? 'is-active' : ''}" href="resource-overview.html" ${page === 'resource-overview' ? 'aria-current="page"' : ''}>${icon('overview')}<span>资源文档概览</span></a>`;
    const firstLevelLinks = [
      ...
      ['resource-catalog-design', '资源目录管理', 'resource-catalog-design.html', 'management', page === 'resource-catalog-design'],
      ['metadata-standards', '元数据标准管理', 'metadata-standards.html', 'management', page === 'metadata-standards'],
    ...
    const firstLevelLabels = { 'resource-tasks': '任务中心', 'data-tables': '数据表', 'graph-data': '图数据', 'resource-catalog-design': '资源目录管理', 'metadata-standards': '元数据标准管理', 'resource-tags': '标签管理', 'knowledge-sources': '知识源管理' };
  ...
  const standaloneSections = {
    ...
    ...Object.fromEntries(applicationChildren.map(([key]) => [key, ['application', '知识应用', 'pages/agent-access.html', 'agent', applicationChildren]]))
  };
  ...
  const topNavigation = `<a href="${href('index.html', root)}">工作台</a>${primaryMenus.map(([key, label, path]) => `<a class="${key === primary[0] && !applicationPageKeys.has(page) ? 'is-active' : ''}" href="${href(path, root)}" ${key === primary[0] && !applicationPageKeys.has(page) ? 'aria-current="page"' : ''}>${label === '知识加工' ? '知识生产' : label}</a>`).join('')}<a class="${applicationPageKeys.has(page) ? 'is-active' : ''}" href="${href('pages/agent-access.html', root)}" ${applicationPageKeys.has(page) ? 'aria-current="page"' : ''}>知识应用</a>`;
